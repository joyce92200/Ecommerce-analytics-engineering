...
Medallion architecture · tested transformations · business-ready marts.
...
| **Silver** (staging) | Type enforcement, deduplication, null handling | `stg_orders` |
| **Gold** (marts) | Business semantics, dimensional model | `dim_users`, `fct_orders`, three analytical marts |
...
Star schema. One fact, one dimension, three derived marts.
Grain: one row per order. Same model answers cohort, refund, and channel questions without rework.

- `dim_users` — one row per user; attributes (country, platform, product, loyalty) locked at acquisition time
- `fct_orders` — one row per order; foreign key to `dim_users`; cohort enrichment computed once
...
*Roadmap: promote `product_name`, `country_code`, and `purchase_platform` to `dim_product`, `dim_country` (with region rollup from `country_lookup_raw`), and `dim_platform` for full snowflake normalization.*
...
Tests run on every model build. 22 assertions across schema, uniqueness, referential integrity, domain values, and derived-column invariants. All passing.

| Test type | Examples | Coverage |
|---|---|---|
| `not_null` | `dim_users.user_id`, `fct_orders.order_id`, `purchase_ts` | Schema integrity |
| `unique` | `dim_users.user_id`, `fct_orders.order_id` | Primary key contracts |
| `relationships` | `fct_orders.user_id` → `dim_users.user_id` | Referential integrity |
| `accepted_values` | `purchase_platform ∈ {website, mobile app}` | Domain constraints |
| `range` | `retention_pct ∈ [0, 100]` | Bounds checking |
| `invariants` | `refunded_revenue + net_revenue = gross_revenue` | Derivation correctness |
...
- 4 product × country segments roll up under NULL `country_code` (orders with missing geolocation); retained as a distinct segment so refund leakage from unknown-country orders remains visible
...
**Roadmap:** dbt Core migration · GitHub Actions CI · BI dashboard layer
...
├── tests/
│   └── test_models.sql       # 22 data-quality assertions
└── docs/                     # Methodology, decisions, lineage

## What's Next

dbt migration · GitHub Actions CI · SCD Type 2 on `dim_users` · BI dashboard layer · forecasting module on platform revenue · promotion of denormalized columns to standalone dimensions
